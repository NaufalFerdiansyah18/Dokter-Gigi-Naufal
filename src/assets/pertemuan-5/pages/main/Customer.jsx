...
export default function Customer() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setData([
      { ...form, id: `#CUS-${String(data.length + 1).padStart(3, "0")}` },
      ...data,
    ]);
    setForm(EMPTY_FORM);
    setShowModal(false);
  ...
  const columns = [
    ...
    { key: "phone", label: "Phone",  render: (v) => <span className="text-gray-500">{v || "-"}</span> },
    {
      ...
      label: "Loyalty",
      render: (val) => {
        const l = LOYALTY_CONFIG?.[val] || {};
        return (
          <Badge variant={LOYALTY_VARIANT[val] || "default"} dot>
            {val}
          </Badge>
        );
      },
    ...
    <div className="flex flex-col w-full gap-4">
      ...
      <Card padding="sm">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <Select
            options={[
              { value: "",       label: "Semua Loyalty" },
              { value: "Bronze", label: "Bronze" },
              { value: "Silver", label: "Silver" },
              { value: "Gold",   label: "Gold" },
            ]}
            value={filterLoy}
            onChange={(e) => { setFilterLoy(e.target.value); setCurrentPage(1); }}
            placeholder={null}
            fullWidth={false}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" type="submit" form="customer-form">Save</Button>
          </>
        }
      >
